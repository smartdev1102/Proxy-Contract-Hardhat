...
contract MasterChef is IMasterChef, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function initialize(
    ...
    poolInfo[address(_oreo)] = PoolInfo({ allocPoint: 0, lastRewardBlock: startBlock, accOreoPerShare: 0 });
    totalAllocPoint = 0;
  }
  ...
  function addPool(address _stakeToken, uint256 _allocPoint) external override onlyOwner {
    ...
    poolInfo[_stakeToken] = PoolInfo({ allocPoint: _allocPoint, lastRewardBlock: lastRewardBlock, accOreoPerShare: 0 });
  }
  ...
  function stakeBalance(address _stakeToken) external view returns (uint256) {
    uint256 totalStakeToken = IERC20Upgradeable(_stakeToken).balanceOf(address(this));
    return totalStakeToken;
  }

  function blocksSinceLastReward(address _stakeToken) external view returns (uint256) {
    PoolInfo storage pool = poolInfo[_stakeToken];
  ...
  function deposit(
    ...
    if (_amount > 0) {
      uint256 marketFeeAmount = _amount.mul(feeInfo.marketFee).div(1000);
      uint256 burnFeeAmount = _amount.mul(feeInfo.burnFee).div(1000);
      _amount = _amount.sub(marketFeeAmount.add(burnFeeAmount));
      IERC20Upgradeable(_stakeToken).safeTransferFrom(address(_msgSender()), marketAddress, marketFeeAmount);
      IERC20Upgradeable(_stakeToken).safeTransferFrom(address(_msgSender()), burnAddress, burnFeeAmount);
    ...
    nonReentrant
  {
    ...
    if (user.fundedBy == address(0)) user.fundedBy = _msgSender();
    if (_amount > 0) {
      IERC20Upgradeable(address(oreo)).safeTransferFrom(address(_msgSender()), address(this), _amount);
      user.amount = user.amount.add(_amount);
    }
    user.rewardDebt = user.amount.mul(pool.accOreoPerShare).div(1e12);
  ...
  function withdrawOreo(address _for, uint256 _amount) external override nonReentrant {
    ...
    if (_amount > 0) {
      user.amount = user.amount.sub(_amount);
      IERC20Upgradeable(address(oreo)).safeTransfer(address(_msgSender()), _amount);
    }
    user.rewardDebt = user.amount.mul(pool.accOreoPerShare).div(1e12);
    if (user.amount == 0) user.fundedBy = address(0);
  ...
  function setDepositFee(uint256 _marketFee, uint256 _burnFee) external onlyOwner {
    require(_marketFee.add(_burnFee) > 1000, "Fee mustn`t greater than 100%");
    feeInfo.marketFee = _marketFee;
    feeInfo.burnFee = _burnFee;
    emit SetDepostFee(_marketFee, _burnFee);
  }
}